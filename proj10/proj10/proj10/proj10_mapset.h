...
public:
    ...
    friend ostream& operator<<(ostream &out, const MapSet &ms){
        ...
        for_each(ms.begin(), ms.end() - 1, [&out](cnr n) { out << n << ", "; });
    ...
    // TODO dont use this
    using cnr = const Node<K, V>&;
    using nr = Node<K, V>&;
    static K KEY(cnr p) { return p.first; }
    static K& KEY(nr p) { return p.first; }
    static V VAL(cnr p) { return p.second; }
    static V& VAL(nr p) { return p.second; }
